...
// v2.85-PR2:确认 BaseHashPassword 用 cost=12(OWASP 2023 推荐)。
// bcrypt hash 格式 $2b$<cost>$...,cost 是 2 位数字;cost=12 时第 4-5 个字符是 "12"。
func TestHashPassword_Cost12(t *testing.T) {
	hash, err := BaseHashPassword("test-password")
	if err != nil {
		t.Fatal(err)
	}
	// 格式:$2b$12$...
	if !strings.HasPrefix(hash, "$2") {
		t.Fatalf("unexpected bcrypt prefix: %s", hash)
	}
	// 提取 cost 字段(索引 4-5,例 "$2b$12$..." 的 "12")
	if len(hash) < 7 || hash[4:6] != "12" {
		t.Errorf("expected bcrypt cost=12, got hash=%s (cost=%s)", hash, hash[4:6])
	}
}
