...
type PageMeta struct {
	...
	// v2.86-PR-Fix (CSP nonce):本请求的 CSP nonce (per-request,16B base64)。
	// 由 secureHeaders 中间件生成,RenderPage 期间从 r.Context() 读出注入到这里。
	// layout.html 里用 {{.CSPNonce}} 给 <script nonce="..."> 用。
	CSPNonce string
}
...
func (s *Server) RenderPage(w http.ResponseWriter, r *http.Request, status int, page string, data interface{}) {
	...
	if r != nil {
		if err := setCSPNonce(data, CSPNonceFromCtx(r.Context())); err != nil {
			if s.Logger != nil {
				s.Logger.Error("set CSPNonce", "page", page, "err", err)
			}
			// 不致命 — nonce 为空只是降级成原 CSP 'self' 行为
		}
	}
...
// setCSPNonce 把 per-request CSP nonce 注入到 data 的 PageMeta.CSPNonce。
//
// 跟 setBodyHTML 模式一致:先看顶层 CSPNonce 字段,否则遍历嵌入字段找 PageMeta.CSPNonce。
// data 必须是 non-nil pointer(由 RenderPage 包装)。
//
// 没有 CSPNonce 字段 → 返回 error,RenderPage 选择忽略(降级成原 CSP 'self' 行为,
// inline script 被拒,但页面不 panic)。这样新增 page data 类型即使忘了嵌入 PageMeta,
// 也不会引发渲染崩溃。
func setCSPNonce(data interface{}, nonce string) error {
	v := reflect.ValueOf(data)
	if v.Kind() != reflect.Ptr || v.IsNil() {
		return fmt.Errorf("data is not a non-nil pointer")
	}
	...
	// 1. 顶层 CSPNonce
	if f := v.FieldByName("CSPNonce"); f.IsValid() {
		if !f.CanSet() {
			return fmt.Errorf("CSPNonce field not settable")
		}
		f.Set(reflect.ValueOf(nonce))
		return nil
	}

	// 2. 嵌入字段 PageMeta.CSPNonce
	for i := 0; i < v.NumField(); i++ {
		f := v.Field(i)
		if !f.CanAddr() {
			continue
		...
		if v.Type().Field(i).Anonymous && f.Kind() == reflect.Struct {
			embedded := f.FieldByName("CSPNonce")
			if embedded.IsValid() && embedded.CanSet() {
				embedded.Set(reflect.ValueOf(nonce))
				return nil
			}
		}
	}
	return fmt.Errorf("data has no CSPNonce field (neither top-level nor in embedded PageMeta)")
}
