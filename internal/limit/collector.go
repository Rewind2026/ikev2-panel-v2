...
//
// activeStates v2.85-PR6 (Q9-02):活跃 SA 状态白名单。
//
// 参考 strongSwan VICI 协议:
//   - ESTABLISHED:正常活跃 SA → 计入流量
//   - REKEYING:正在重新协商密钥,旧 SA 仍传输流量 → **本次新增,计入**
//   - REKEYED:新 SA 已建立,旧 SA 准备删除(流量已切到新 SA) → 跳过(避免双计)
//   - CONNECTING / DELETING / INSTALLED:中间态 → 跳过
//
// 用 map 白名单而非 `if state != "ESTABLISHED" && state != "REKEYING"`:
// 后续加新状态(如有)只改一处。
package limit
...
func (c *Collector) collect(ctx context.Context) {
	...
	for _, sa := range sas {
		// v2.85-PR6 (Q9-02):用 activeStates 白名单替代原来的 "if IkeState != ESTABLISHED"。
		// REKEYING 状态 SA 也计入流量(旧 SA 仍在传输数据)。
