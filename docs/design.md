...
### 15.2 出向路由表 220 + FORWARD ACCEPT + MASQUERADE（v2-78 新增，v2-79 自动化）
...
## 18. 网络模式自动选择（v2-79 文档化，v2-80+ 实现）

### 18.1 四种模式对比

| 模式 | 适用场景 | 公网 IP | 端口冲突风险 | v2-79 支持 |
|---|---|---|---|---|
| **`network_mode: host`** | 主流推荐，宿主有公网 IP | ✅ 直绑 | ⚠️ UDP 500/4500 与宿主机其他服务冲突 | ✅ 默认 |
| **`bridge` + libipsec** | 端口冲突时 fallback；ESP 走 UDP/4500 绕过 XFRM | ⚠️ 需要端口映射 | ✅ 无冲突 | 🟡 docker-compose 注释模板可用，需手改 sw |
| **`bridge` + ipvlan** | 想做网络隔离 + 有公网 IPv6 子网 | ✅ | ⚠️ 需手工建 ipvlan 网络 | 🟡 docker-compose 注释模板可用 |
| **`bridge` + macvlan** | fallback for ipvlan 不支持的环境 | ⚠️ 父接口是宿主物理网卡 | — | ❌ 不提供模板 |

**v2-79 默认 host 网络**（docker-compose.yml `network_mode: host` 保留注释中的 ipvlan 桥段作为示例）。

### 18.2 自动选择策略（v2-79 设计，v2-80+ 实现）

```bash
# 探测逻辑（v2-80+ 实现）
if OUT_IF 上有 2000::/3 公网 IPv6：
    IKEV2_NETWORK_MODE=host      # 推荐：直绑宿主网卡
elif OUT_IF 上有公网 IPv4：
    IKEV2_NETWORK_MODE=bridge+libipsec  # 需要端口映射 500:500/udp 4500:4500/udp
else:
    IKEV2_NETWORK_MODE=bridge+ipvlan    # 让用户做 ipvlan 网络
```

### 18.3 当前 v2-79 限制

- `IKEV2_NETWORK_MODE=auto` 环境变量已添加到 docker-compose.yml，但 entrypoint 还没真正用做网络模式切换（docker compose 启动前就要决定 network_mode，entrypoint 无法热切换）
- §0.7 自动选网络模式只在设计文档里写明，**v2-79 的实现只做了 host 网络下的零硬编码**
- bridge/ipvlan 模式的自动切换在 v2-80 计划
- 当前如果用户不用 host 网络，需要手动改 docker-compose.yml 注释 + 自己建 ipvlan 网络（模板已在 docker-compose.yml 注释里给）
...
DDNS 涉及：
- 用户的 DNS provider（aliyun / cloudflare / godaddy ... 各自 API 不同）
- 域名所有权校验
- 续签与 TLS 证书同步（acme.sh 已经覆盖）

**v2-79 把 DDNS 留作"未来扩展点"**——`ipv6watch.go` 已经能感知 IP 变化，挂个 aliyun API 调用就能加，但需要用户明确决定 DNS provider。
...
## 21. v2-79 → v2-80 路线图（不在本次范围）

| 项 | 优先级 | 触发条件 |
|---|---|---|
| §0.7 自动选网络模式（host/bridge/ipvlan） | 中 | 用户明确要求非 host 部署 |
| DDNS 同步（aliyun/cloudflare API） | 中 | 用户明确指定 DNS provider |
| IPv6 限速（tc 替代方案：nftables + cgroups） | 低 | 用户要求 IPv6 限速 |
